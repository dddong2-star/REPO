    ...
    with tqdm(total=num_episodes, desc="Training") as pbar:
        ...
        for i_episode in range(num_episodes):
            ...
            if (i_episode+5) % 10== 0:
                # 10轮评估一次
                print("=======评估=========")
                best_p,best_r,best_f1, best_prompt, best_actions = evaluate(q_network, state_model, reward, val_dataset, initial_prompt,model_save_path,i_episode)
                print("======评估结果=======")
                # 打印评估结果
...
def evaluate(q_network, state_model, reward, val_dataset, prompt,model_save_path,epoch):
    ...
    while stable_count <= 3:
        ...
        if avg_f1 >= best_f1:
            ...
            save_path = f"{model_save_path}_{epoch}.pth"
